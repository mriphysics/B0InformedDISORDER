function [Tou,F,NE, cou, cou_idx]=compressMotion(Tin,F,res,parXT, cin, disableGrouping)
...
if nargin < 5 || isempty(cin); cin = zeros(size(Tin)); end 
...
if NT(5)>1
    ...
    while n<=NT(5)
        ...
        while m+n<=NT(5)
            ...
            if any(abs(Tnex(:)-Tcur(:))>(-1)^disableGrouping * 1e-6);break;end 
            m=m+1;
        end
        indGroup(n:n+m-1)=m;
        n=m+n;
    ...
    while n<=NT(5)
        ...
        if ~disableGrouping
            ...
            Tou=dynInd(Tou,nb,5,mean(dynInd(Tin,nG,5),5));
        end
        cou = dynInd(cou,nb,5,mean(dynInd(cin,nG,5),5));
        cou_idx = dynInd(cou_idx,nG,5,nb); % nb*ones(size(nG))
        nGr=nG(nG>nb);        
