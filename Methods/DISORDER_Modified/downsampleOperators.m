...
NXRes=ceil(NX(1:3)./(2.^l));NYRes=ceil(NY(1:3)./(2.^l));%YB: I don't know if NXRes and NYRes should have different dimensions
...
if useFilter && gibbsRing~=0 && gibbsRing<=1;HY=buildFilter(NYRes(1:3),'tukeyIso',[],gpuIn,gibbsRing);elseif useFilter && l~=0;HY=buildFilter(NYRes,'CubicBSpline',[],gpuIn);else HY=[];end
if ~isempty(HY)
